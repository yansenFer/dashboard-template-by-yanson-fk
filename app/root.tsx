...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "Ferforge UI - Template Dashboard" },
    {
      name: "description",
      content:
        "Premium dashboard template built with React Router and Tailwind CSS",
    },
  ];
}
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  useEffect(() => {
    ...
    try {
      // Read from Redux store first, then fallback to localStorage or system preference
      const reduxDark = store.getState().dark.isDark;
      const localDark = localStorage.getItem("isDark");
      if (localDark !== null) {
        activeDark = localDark === "true";
      } else {
        activeDark =
          reduxDark ||
    ...
    setTheme(activeDark ? "dark" : "light");
    if (activeDark) {
